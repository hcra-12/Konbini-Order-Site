...
public class OrderController {
	...
	public String confirm(@RequestParam("GN") List<String> GN,
		...
		for(int i = 0; i < ON.size(); i++) {
			...
			jdbcTemplate.update("INSERT INTO ordergoods(Order_Name,Order_Image_Name,Order_Num,Order_Date_Time,Order_Date) values (?,?,?,?,?)" ,
					orderName,orderNameImage,orderNum,dateTime.format(D),df.format(D));
		}
		return "OrderConfirm";
	}
}
